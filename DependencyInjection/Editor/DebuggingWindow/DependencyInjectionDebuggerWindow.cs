...
{
    public class DependencyInjectionDebuggerWindow : EditorWindow
    {
        ...
        private static List<(IResolver, Type[])> BuildMatrix(Container container)
        {
            ...
            if (DependencyInjectionEditorSettings.ShowInternalBindings == false)
            {
                var containerResolver = container.ResolversByContract[typeof(Container)].Single();
                resolvers.Remove(containerResolver);
            }

            if (DependencyInjectionEditorSettings.ShowInheritedBindings == false && container.Parent != null)
            {
        ...
        private void OnGUI()
        {
            ...
            if (UnityScriptingDefineSymbols.IsDefined("DependencyInjection_DEBUG"))
            {
        ...
        {
            GUILayout.FlexibleSpace();
            GUILayout.Label("To start debugging, enable the DependencyInjection Debug Mode in the status bar.", Styles.LabelHorizontallyCentered);
            GUILayout.Label("Keep in mind that enabling DependencyInjection Debug Mode will impact performance.", Styles.LabelHorizontallyCentered);
        }
        ...
        private void PresentHierarchyFilters()
        {
            ...
            DependencyInjectionEditorSettings.ShowInternalBindings = GUILayout.Toggle(DependencyInjectionEditorSettings.ShowInternalBindings, "Show Internal Bindings ");
            DependencyInjectionEditorSettings.ShowInheritedBindings = GUILayout.Toggle(DependencyInjectionEditorSettings.ShowInheritedBindings, "Show Inherited Bindings ");
        ...
        private void PresentStatusBar()
        {
            using (new EditorGUILayout.HorizontalScope(Styles.AppToolbar))
            {
                ...
                var debuggerIcon = UnityScriptingDefineSymbols.IsDefined("DependencyInjection_DEBUG")
                    ? EditorGUIUtility.IconContent("d_DebuggerEnabled")
                    : EditorGUIUtility.IconContent("d_DebuggerDisabled");

                debuggerIcon.tooltip = UnityScriptingDefineSymbols.IsDefined("DependencyInjection_DEBUG")
                    ? "DependencyInjection Debugger Enabled"
                    : "DependencyInjection Debugger Disabled";
                
                if (GUILayout.Button(debuggerIcon, Styles.StatusBarIcon, GUILayout.Width(25)))
                {
                    UnityScriptingDefineSymbols.Toggle("DependencyInjection_DEBUG", EditorUserBuildSettings.selectedBuildTargetGroup);
                }
            }
        }
